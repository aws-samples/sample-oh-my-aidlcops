...
`oh-my-aidlcops`(OMA)는 AIDLC(AI-Driven Development Lifecycle)의 전 단계(Inception → Construction → Operations)를 에이전트 기반 운영 자동화로 연결하는 **플러그인 마켓플레이스**입니다. OMA는 [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)(OMC)의 형제 프로젝트이며, 범용 Claude Code 오케스트레이션 철학을 AIDLC 전용 도메인으로 확장합니다. 핵심 명제는 단순합니다 — **AIDLC는 운영이 에이전트로 자동화되었을 때 비로소 완성됩니다.**
...
플러그인 상세 정의는 리포지터리 루트의 [`.claude-plugin/marketplace.json`](https://github.com/aws-samples/sample-oh-my-aidlcops/blob/main/.claude-plugin/marketplace.json)에 있습니다.
...
OMA는 재발명 대신 재사용을 원칙으로 합니다. 전체 attribution은 [NOTICE](https://github.com/aws-samples/sample-oh-my-aidlcops/blob/main/NOTICE)에 문서화되어 있습니다.
...
| [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) | — | Tier-0 오케스트레이션 철학·`.omc/` 상태 관리 계승 |
...
### 관련 프로젝트
- [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) — 범용 Claude Code 오케스트레이션 (OMA의 모체)
