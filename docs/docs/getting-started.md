...
OMA는 프로젝트별 상태를 `.omao/` 디렉터리에 보관합니다. **`oma setup` 을 실행했다면
이 단계는 이미 완료된 상태이므로 건너뛰셔도 됩니다** (setup 이 내부적으로
`init-omao.sh` 를 호출해 `.omao/` 를 초기화합니다).

`oma setup` 없이 `.omao/` 만 만들고 싶다면 다음을 실행합니다.

```bash
# 원격 install.sh 로 설치한 경우 (권장)
cd <your-project>
...
> `<oma-repo>` 가 아니라 **실제 경로**를 입력합니다. `install.sh` 로 설치했다면
> `~/.oma`, git clone 했다면 clone 디렉터리를 넣어주세요.
