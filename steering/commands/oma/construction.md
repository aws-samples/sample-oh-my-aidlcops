...
---
<objective>
aidlc 플러그인 construction 스킬을 순서대로 실행해 컴포넌트 설계, 코드 스캐폴딩, 테스트, PR 초안을 생산한다. 기존에 정의된 스펙·유저스토리·workflow plan을 그대로 소비하며 구현 일관성을 유지한다.
</objective>
...
2. **Pre-flight Checks** — Inception 산출물 존재 여부, aidlc 플러그인 (construction 스킬 그룹) 설치, 테스트 러너·빌드 도구 동작 확인
3. **Plan** — 컴포넌트 설계 다이어그램, 파일 생성 경로, TDD 테스트 케이스 목록을 사용자와 합의
4. **Execute** — aidlc 플러그인 construction 스킬을 다음 순서로 호출
...
- aidlc 플러그인 (construction 스킬 그룹) 설치 (`plugins/aidlc/`)
</prerequisites>
